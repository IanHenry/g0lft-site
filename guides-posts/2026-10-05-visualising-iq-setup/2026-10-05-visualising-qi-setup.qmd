...
- Download the latest installer from the [Radioconda GitHub](https://github.com){:target="_blank"}.
...
- If you are using hardware other than an SDRplay, refer to the [Radioconda ReadMe page](https://github.com#additional-installation-for-device-support){:target="_blank"} for further guidelines.
...
You are now ready to use GNU Radio to process the IQ output of your SDR receiver. The learning curve can be steep, so I recommend checking out the [GNU Radio wiki](https://gnuradio.org){:target="_blank"}. To get you started, I have included several sample flowgraphs in my repository for you to modify and explore. If you have questions, feel free to [email me](mailto:YOUR_EMAIL_HERE).

## Sample FlowGraphs

These flowgraphs are provided "as-is" in the following Git repository: [**INSERT_FLOWGRAPH_REPO_URL_HERE**]. Many allow you to capture raw IQ to a file, which can then be processed with the included Python scripts to create high-quality density plots.
...
- Download [SDRconnect](https://sdrplay.com){:target="_blank"} for Windows, MacOS, or Linux. Ensure you are using at least version 1.08.
...
- Download my script from [**INSERT_PLUGIN_REPO_URL_HERE**] and run ```python sdrConnectIQvisualiser.py```. It does not need to be in the SDRconnect directory as it communicates via the network API.
- **Windows users:** A pre-compiled .exe version is also available at [**INSERT_EXE_DOWNLOAD_URL_HERE**] for a standalone launch.

### Using the Simple IQ Visualisation Plugin

This tool is designed for visualising strong signals like Broadcast FM/AM, Repeaters, and Airband. It uses the sampled window centered on the waterfall. While audio filters are applied in SDRconnect, they do not affect this raw IQ stream. The script automatically periodically re-centers the VFO so the IQ you see matches your tuned frequency.
...
I built this tool in an afternoon to give newcomers an easy starting point—enjoy exploring the signals!
