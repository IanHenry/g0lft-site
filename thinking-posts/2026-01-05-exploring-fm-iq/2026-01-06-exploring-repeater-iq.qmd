...
The central cloud corresponds to periods when the repeater is idle. With no strong carrier present, the IQ samples collapse toward the origin and form a roughly circular noise distribution.
...
During periods when the repeater is transmitting, the unwrapped phase increases steadily, just as in the broadcast FM case. During silence, the phase becomes essentially random, reflecting the absence of a coherent carrier.
...
When instantaneous frequency is plotted, the spoken time announcement becomes clearly visible. Speech appears as deviations around the centre frequency, while silence corresponds to a flat or noisy trace.
...
Demodulating the signal produces the expected result: silence, followed by the time announcement, followed by noise as the repeater drops.

As before, this is included only as a reference. Everything important about the signal’s behaviour has already been visible in the IQ-derived plots.

## Audio-synchronised IQ animation (supplementary)
